...
## Forensic Integration 1/5: This integration will check for any suspecious commits made between 24 ~ 8.
def getAllCommits(all_repos): 
    ...
    all_days       = []
    tracker        = 0
    suspecious_commits = [] ## A list that holds any suspecious commits. In this case, we decided to commits any commits during unsual time.
    for repo_ in all_repos:
        ...
        ## For loop to check commits.
        for commit in commits:
            c_time = pd.to_datetime(commit['timestamp']).hour
            if c_time < 8 or c_time > 23: ## Any commits made between 24 ~ 8 will be considered suspecious.
                suspecious_commits.append(commit)
    ...
    #### This will print out any suspecious commit times.
    print("Suspecious commits made during these hours: ")
    for commit in suspecious_commits:
        print(commit)
    ####
...
## Forensic Integration 2/5: This integration will check for any devs with low count of commits. 
def getDevEmails(ds_list):
    commit_count = defaultdict(int) ## Made to count # of commits made by each Dev.
    repo_emails = []
        ...
        for full_path_to_repo in all_repos:
            ...
            if os.path.exists(full_path_to_repo):
                ...
                for commit_ in all_commits:
                    commit_hash = commit_.hexsha
                    emails = getDevEmailForCommit(full_path_to_repo, commit_hash)
                    for email in emails: ## This will increase commit.
    ...
    ## This will output any dev that has lower than 3 commits.
    low_commits_dev = [email for email, count in commit_count.items() if count <3]
    print('Devs with low counts: ', low_commits_dev)
    return low_commits_dev
